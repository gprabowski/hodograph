...
void hodograph_scene::render(input_state &input) {
  ...
  // FIRST APPLY THE NORMALLY DISTRIBUTED CHANGE
  static std::random_device rd{};
  static std::mt19937 gen{rd()};
  std::normal_distribution<float> d{0.f, model.hodograph.epsilon0};
  model.hodograph.l2 += d(gen);
  ...
  const std::chrono::duration<float> time_delta =
  ...
  // FIND L2 CONFIG
  const auto delta = 4 * sq(model.hodograph.l1) *
                         std::cos(model.hodograph.angle) *
                         std::cos(model.hodograph.angle) -
  ...
  const auto first_sol =
      (2 * model.hodograph.l1 * std::cos(model.hodograph.angle) + sq_delta) / 2;
  const auto second_sol =
      (2 * model.hodograph.l1 * std::cos(model.hodograph.angle) - sq_delta) / 2;
  ...
  auto beta =
      std::acos((sq(model.hodograph.l2) + sq(l3) - sq(model.hodograph.l1)) /
                (2 * model.hodograph.l2 * l3));

  if (std::fmod(model.hodograph.angle, 2 * glm::pi<float>()) <
      glm::pi<float>()) {
  ...
  render_element(model.renderable.unit_cyllinder, model.geometry.unit_cyllinder,
                 mmat);

  mmat = math::get_model_matrix(
             {std::cos(model.hodograph.angle) * model.hodograph.l1,
              std::sin(model.hodograph.angle) * model.hodograph.l1, 0.f},
             {1.f, 1.f, 1.f}, glm::vec3{0.f, 0.f, beta}) *
         math::get_model_matrix({0.f, 0.f, 0.f}, {model.hodograph.l2, 1.f, 1.f},
                                glm::vec3{0.f, 0.f, 0.f});
